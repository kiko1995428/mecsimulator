...
from CloudletSimulator.simulator.model.edge_server import MEC_server, cover_range_search
import pandas as pd
import pickle
#CSV読み込み
...
for i in range(num):
    ...
    for index, series in df.iterrows():
        mec[index] = MEC_server(resource, index + 1, server_type, series["lon"], series["lat"],
                                cover_range)  # ０からカウントのため、1を＋
        ...
        memo = cover_range_search(device_flag, device_lon, device_lat, mec[index].lon, mec[index].lat,
                                              cover_range, index + 1)
        if memo > 0:
            tmp = memo
        #print("**************")
    print("車両ID：", i, ",", "correct ID is ", tmp)
