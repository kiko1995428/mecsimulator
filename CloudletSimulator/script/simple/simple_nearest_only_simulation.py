...
def nearest_simulation(system_end_time, MEC_resource, device_num, device_allocation_method, path_w, how_compare):
    ...
    if device_allocation_method == 0:
        if how_compare == "hop":
            d = open('/Users/sugimurayuuki/Desktop/mecsimulator/CloudletSimulator/dataset/device.simple_congestion_binaryfile_500', 'rb')
        else:
    ...
    # 混雑度順
        ...
        if how_compare == "hop":
            cd = open('/Users/sugimurayuuki/Desktop/mecsimulator/CloudletSimulator/script/simple/device_compare/congestion_checked_devices2.binaryfile', 'rb')
        else:
            cd = open('/Users/sugimurayuuki/Desktop/mecsimulator/CloudletSimulator/script/simple/device_compare/congestion_checked_devices1.binaryfile','rb')
        cd = pickle.load(cd)
        devices = cd
        num = len(devices)
    ...
    for t in range(system_end_time):
        ...
        for i in range(num):
            ...
            if check_between_time(sorted_devices[t][i], t) == True:
                ...
                # 最近傍割り当て処理
                device_flag, allocation_MEC_name = nearest_search2(sorted_devices[t][i], mec, sorted_devices[t][i].plan_index, cover_range, t)
                # 最近傍割り当てが成功したら表示する
                if device_flag == True:
                ...
                if sorted_devices[t][i].mec_name != [] and sorted_devices[t][i]._lost_flag == False:
                    previous_index = search_mec_index(mec, sorted_devices[t][i].mec_name)
                    print("DECREASE")
